...
export const CODE_TABLES = {
  bcd: { nameKey: 'code.bcd', bits: 4, of: d => d.toString(2).padStart(4, '0') },
  '2421': {
    nameKey: 'code.2421',
    bits: 4,
  ...
  excess3: { nameKey: 'code.excess3', bits: 4, of: d => (d + 3).toString(2).padStart(4, '0') },
};

/** Mã của một chữ số 0..9 theo một bảng mã. */
export function encodeDigit(d, table) {
  const t = CODE_TABLES[table];
  if (!t) fail('err.noCodeTable', { table });
  if (!Number.isInteger(d) || d < 0 || d > 9) fail('err.digit0to9');
  return t.of(d);
}
...
export function encodeDecimal(text, table) {
  ...
  if (!/^\d+$/.test(s)) fail('err.decimalOnly');
...
export function addBcdDigits(a, b, carryIn = 0) {
  ...
  const steps = [
    { labelKey: 'bcd.binSum', value: raw + '  (' + sum + ')' },
    needsFix
      ? { labelKey: 'bcd.needFix', value: '+ 00110' }
      : { labelKey: 'bcd.noFix', value: '—' },
    { labelKey: 'bcd.result', value: (carry ? '1 ' : '') + digit.toString(2).padStart(4, '0') + '  (' + digit + ')' },
  ];
...
export function addBcd(aText, bText) {
  ...
  if (!/^\d+$/.test(A) || !/^\d+$/.test(B)) fail('err.decimalOnly');
...
export function checkParity(bitsWithParity, kind = 'even') {
  if (!/^[01]+$/.test(bitsWithParity)) fail('err.bitsOnly');
...
export function asciiBits(ch) {
  ...
  if (!(code >= 0 && code <= 127)) fail('err.asciiOnly');
